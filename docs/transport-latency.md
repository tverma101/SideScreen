...
SideScreen treats the Mac-to-tablet path as a freshness-first monitor
...
`/tmp/sidescreen.log` file is written by a bounded asynchronous writer, so the
...
```bash
defaults delete com.sidescreen.app SideScreen_exp_bitrate
```
