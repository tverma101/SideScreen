...
APP_PATH="/Users/$USER/Applications/SideScreen.app"
SCENARIO="manual"
DURATION=60
...
if [[ -z "$HOST_PID" ]] && (( LAUNCH_HEADLESS )); then
  if [[ ! -x "$APP_PATH/Contents/MacOS/SideScreen" ]]; then
    echo "Installed executable not found: $APP_PATH/Contents/MacOS/SideScreen" >&2
    exit 3
  fi
  "$APP_PATH/Contents/MacOS/SideScreen" --headless >/tmp/sidescreen-host-lab.stdout 2>/tmp/sidescreen-host-lab.stderr &
  started_pid=$!
...
if [[ -z "$HOST_PID" ]]; then
  for name in SideScreen "Side Screen" SideScreenHost; do
    HOST_PID="$(pgrep -x "$name" 2>/dev/null | head -n 1 || true)"
    [[ -n "$HOST_PID" ]] && break
  done
fi
if [[ -z "$HOST_PID" ]] || ! kill -0 "$HOST_PID" 2>/dev/null; then
  echo "Could not identify SideScreen. Set --pid or use --launch-headless." >&2
  exit 4
fi

host_command="$(ps -p "$HOST_PID" -o command= | sed 's/^[[:space:]]*//')"
if [[ "$host_command" != *"SideScreen"* ]]; then
  echo "PID $HOST_PID is not a SideScreen executable: $host_command" >&2
  exit 5
fi
...
for ((sample = 0; sample < DURATION; sample++)); do
  if ! kill -0 "$HOST_PID" 2>/dev/null; then
    echo "SideScreen exited during sample $sample" >&2
    break
  fi
  now="$(date +%s)"
