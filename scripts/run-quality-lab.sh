...
# already started the exact SideScreen host profile and tapped Connect; this
# script never fakes a UI tap or changes production defaults. The host pattern
# injection keys are documented in docs/eval/labs.md and are intentionally
# opt-in for a run.
...
SideScreen SurfaceView with PixelCopy; the host must already be streaming the
same pattern.
EOF
}
...
QUALITY_LAB=(swift run -c release --package-path "$ROOT_DIR/MacHost/QualityLab" sidescreen-quality-lab)
...
if [[ "$MODE" == streamed || "$MODE" == both ]]; then
  echo "Waiting for an active SideScreen stream; streamed PixelCopy will fail if the SurfaceView is idle." >&2
