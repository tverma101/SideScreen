...
# SideScreen adaptive-refresh benchmark sampler.
#
# Usage:
...
find_host_pid() {
  ...
  pid="$(pgrep -f '/SideScreen([^/]*)\.app/|/MacHost/.build/.*/SideScreen' 2>/dev/null | head -n 1 || true)"
  echo "$pid"
}
...
if [[ -z "$host_pid" ]]; then
  echo "Could not find SideScreen. Set SIDESCREEN_PID=<pid> and retry." >&2
  exit 3
fi
...
if ! kill -0 "$host_pid" 2>/dev/null; then
  echo "SideScreen PID $host_pid is not running." >&2
  exit 5
fi
...
for ((i = 0; i < duration; i++)); do
  if ! kill -0 "$host_pid" 2>/dev/null; then
    echo "SideScreen exited during sample $i" >&2
    break
  fi
...
awk -F, '
  ...
  $5 != "nan" { ws_sum += $5; ws_n++ }
  END {
    if (host_n) printf("SideScreen mean CPU: %.2f%%\n", host_sum/host_n)
...
echo "Also capture SideScreen logs containing 'Adaptive refresh:' for tier transitions."
