...
/// Compact AppKit brightness control embedded in the existing status-item
/// menu. It uses the same persisted level and connection callback as the
/// streaming server, so the tablet can be set before it connects.
final class BrightnessMenuItemView: NSView {
    ...
    init(level: UInt8) {
        ...
        slider.doubleValue = Double(NativeBrightnessController.clampedLevel(Int(level)))
        slider.isContinuous = true
        slider.target = self
        slider.action = #selector(valueChanged(_:))
        slider.setAccessibilityLabel("SideScreen tablet brightness")
        ...
        updateValueLabel(for: UInt8(slider.integerValue))
    }
    ...
    @objc private func valueChanged(_ sender: NSSlider) {
        let level = UInt8(NativeBrightnessController.clampedLevel(sender.integerValue))
