...
/// evaluated under the canonical SideScreen identity rather than an ad-hoc
...
enum CaptureSourceBenchmark {
    ...
    static func runIfRequested(arguments: [String]) -> Bool {
        ...
        emit("This process uses the canonical SideScreen bundle identity for TCC.")
    ...
    private static func runCGDisplayStream(
        ...
        let queue = DispatchQueue(label: "com.sidescreen.capture-benchmark.cg", qos: .userInteractive)
        ...
        Task {
            ...
            do {
                ...
                try stream.addStreamOutput(
                    output,
                    type: .screen,
                    sampleHandlerQueue: DispatchQueue(label: "com.sidescreen.capture-benchmark.sc", qos: .userInteractive)
                )
