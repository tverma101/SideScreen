...
struct ScreenRecordingPermissionSnapshot: Equatable {
    ...
    static func current(preflight: Bool = CGPreflightScreenCaptureAccess()) -> Self {
        ...
        let canonicalPath = FileManager.default.homeDirectoryForCurrentUser
            .appendingPathComponent("Applications/SideScreen.app")
            .path
    ...
    var diagnosticText: String {
        ...
        return "macOS did not authorize this exact running bundle. The Settings switch may belong to another SideScreen copy or an older local rebuild."
    }

    var recoveryText: String {
        if isCanonicalInstall {
            return "Turn SideScreen off and on in Screen & System Audio Recording. If it remains Required, remove the entry and add this exact bundle again."
        }
